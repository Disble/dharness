...
func RunCheck(args []string, stdout io.Writer) error {
	...
	staged, err := project.StagedSourceFiles(dir)
	if err != nil {
		return err
	}
	if len(staged) == 0 {
	...
	stages := []struct {
		...
		{tool.ReactDoctor, tool.ReactDoctorStaged()},
		{tool.Fallow, tool.FallowAudit()},
	}
	...
	for index, stage := range stages {
		...
		command := p.Resolve(stage.tool).Command(dir, stage.args...)
